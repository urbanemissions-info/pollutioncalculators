...
def update_collection_efficiency():
    ...
    st.session_state.collection_efficiency = waste_collection_efficiency_urban * (
        1 + st.session_state.delta_wastecollection / 100
    )
...
def update_burn_rate():
    ...
    st.session_state.burnrate = waste_burn_rate_urban * (
        1 - st.session_state.delta_wasteburn / 100
    )
...
def update_gen_rate():
    ...
    st.session_state.genrate = percapita_waste_gen_urban * (
        1 + st.session_state.delta_wastegeneration / 100
    )
...
pop = st.sidebar.number_input(
    "Population (in millions):", placeholder="Population of the city ...", value=2.3
)

st.sidebar.write("### Urban inputs  :gear:")
urbanshare = st.sidebar.slider(
    "Urban share of population (%)", min_value=0, max_value=100, value=80
)

# URBAN GEN RATE - HAS CALLBACK
if "urban_genrate" not in st.session_state:
    st.session_state.urban_genrate = 0.4  # Default val
percapita_waste_gen_urban = st.sidebar.number_input(
    "Per capita waste generated (urban) (kg/cap/day):",
...
waste_collection_efficiency_urban = st.sidebar.slider(
    "Waste collection efficiency in Urban (%)",
    min_value=0,
    max_value=100,
...
waste_burn_rate_urban = st.sidebar.slider(
    "Waste burn rate in Urban (%)",
    min_value=0,
    max_value=100,
    key="urban_burnrate",
...
landfill_capacity = st.sidebar.number_input(
    "Landfill capacity (tons/day):",
    placeholder="Landfill capacity (tons/day)",
    value=250,
)

landfill_burn_rate = st.sidebar.slider(
    "Landfill burn rate (%)", min_value=0, max_value=100, value=2
)
wet_waste_urban = st.sidebar.slider(
    "Wet waste in Urban (%)", min_value=0, max_value=100, value=30
)

## RURAL INPUTS
st.sidebar.write("### Rural inputs  :gear:")
percapita_waste_gen_rural = st.sidebar.number_input(
    "Per capita waste generated (rural) (kg/cap/day):",
    placeholder="Per capita waste generated (rural) (kg/cap/day)",
    value=0.3,
)


waste_collection_efficiency_rural = st.sidebar.slider(
    "Waste collection efficiency in Rural (%)", min_value=0, max_value=100, value=5
)

waste_burn_rate_rural = st.sidebar.slider(
    "Waste burn rate in Rural (%)", min_value=0, max_value=100, value=100
)
wet_waste_rural = st.sidebar.slider(
    "Wet waste in Rural (%)", min_value=0, max_value=100, value=30
)
...
urban_waste_generated = (
    1000000 * urban_population * urban_genrate / 1000
)  # 1000 - Tonnes per day
rural_waste_generated = (
    1000000 * rural_population * percapita_waste_gen_rural / 1000
)  # Tonnes per day
...
with tab1:
    ...
    with col2:
        ...
        st.metric(
            label="Current Landfill capacity",
            label_visibility="visible",
...
with tab2:
    ...
    increase_waste_collection = st.slider(
        "I want to increase my waste collection efficiency by (%)",
        min_value=0,
        max_value=100,
    ...
    c1, c2 = st.columns(2)
    c1.write(
        "**Old Waste collection Efficiency**: {} %".format(
            round(waste_collection_efficiency_urban)
        )
    )
    c2.write(
        "**New Waste collection Efficiency**: {} %".format(
            round(st.session_state.collection_efficiency)
        )
    )
    ...
    reduce_waste_burn = st.slider(
        "I want to reduce my waste burn rate by (%)",
        min_value=0,
        max_value=100,
        key="delta_wasteburn",
    ...
    change_waste_generation = st.slider(
        "I want to change my waste generate rate by (%)",
        min_value=-100,
        max_value=100,
    ...
    c1, c2 = st.columns(2)
    c1.write(
        "**Old Waste generation rate**: {} (kg/cap/day)".format(
            round(percapita_waste_gen_urban, 2)
        )
    )
    c2.write(
        "**New Waste generation rate**: {} (kg/cap/day)".format(
            round(st.session_state.genrate, 2)
        )
    )
