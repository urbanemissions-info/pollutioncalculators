...
with tab1:
    ...
    with col1:
        ...
        for i in range(num_zones):
            st.text_input(
                "Zone", value=default_values_zones[i], label_visibility="collapsed"
            )
    ...
    with col2:
        ...
        for i in range(num_zones):
            ...
            variables_dict[var_name] = st.number_input(
                "Population", label_visibility="collapsed", key=keyplace
            )
    ...
    with col3:
        ...
        for i in range(num_zones):
            ...
            variables_dict[var_name] = st.number_input(
                "Avg.Conc μg/m3", label_visibility="collapsed", key=keyplace, step=1
            )
        ...
        st.write(
            "#### :green[Population weighted concentration: {} μg/m3]".format(
                round(pop_weighted_conc, 1)
            )
        )
    ...
    with col4:
        ...
        for i in range(num_zones):
            ...
            # if keyplace not in st.session_state:
            #     st.session_state[keyplace] = default_values[i] #Default val
            variables_dict[var_name] = st.slider(
                "Avg.Conc μg/m3", label_visibility="collapsed", key=keyplace
            )
    ...
    with col5:
        ...
        for i in range(num_zones):
            ...
            new_concs.append(
                st.session_state[oldconc_keyplace]
                * (1 - st.session_state[reduction_keyplace] / 100)
            )
        ...
        st.write(
            "#### :green[New Population weighted concentration:  {} μg/m3]".format(
                round(new_pop_weighted_conc, 1)
            )
        )

    with col4:
        net_reduction = round(
            100 * (pop_weighted_conc - new_pop_weighted_conc) / pop_weighted_conc, 1
        )
        st.write("#### :green[Net reduction:  {} %]".format(round(net_reduction, 1)))

# *************** TAB 2 - V2 ***************#
with tab2:
    net_reduction = st.slider(
        "I want to reduce pollution in the city by: (%)", value=30
    )
    ...
    with col4:
        ...
        max_pollution_reduction_defaults = [
            40,
            40,
            40,
            40,
        ...
        for i in range(num_zones):
            ...
            if keyplace not in st.session_state:
                st.session_state[keyplace] = max_pollution_reduction_defaults[
                    i
                ]  # Default val
            variables_dict[var_name] = st.slider(
                "Max pollution reduction", label_visibility="collapsed", key=keyplace
            )
    ...
    with col5:
        ...
        def calculate_pct_reduction(old_pollutions, new_pollutions):
            reductions = [
                100 * (old - new) / old
                for old, new in zip(old_pollutions, new_pollutions)
            ]
            return reductions
...
with tab3:
    ...
    c1, c2 = st.columns(2)
    zone_concentrations_df = pd.read_csv(
        "AQI_Simulator/inputs/zone_concentrations_default.csv"
    )
    sourceapportionment_df = pd.read_csv(
        "AQI_Simulator/inputs/sourceapportionment_default.csv"
    )
    reduction_sourcewise_df = pd.read_csv(
        "AQI_Simulator/inputs/reduction_sourcewise.csv"
    )
    ...
    with c2:
        ...
        reduction_sourcewise_df = st.data_editor(
            reduction_sourcewise_df, column_config=column_config_dict
        )
    ...
    pop_weighted_conc = np.dot(
        zone_concentrations_array.T, zone_populations_array
    ) / np.sum(zone_populations_array)
    ...
    source_concentrations_old = sourceapportionment_array * (
        zone_concentrations_array.T
    )
    ...
    zone_concentrations_df["Avg.conc ug/m3 (new)"] = zone_concentrations_avg_new.round(
        1
    )

    with c1:
        zone_concentrations_df["Zone.pop (mil)"] = zone_concentrations_df[
            "Zone.pop (mil)"
        ].astype(str)
        # Because streamlit better edits floats when they are like strings. We will reconvert them to floats
        zone_concentrations_df = st.data_editor(
            zone_concentrations_df,
            num_rows="dynamic",
            column_config={
                "Zone.pop (mil)": st.column_config.TextColumn("Zone.pop (mil)")
            },
        )
        zone_concentrations_df["Zone.pop (mil)"] = zone_concentrations_df[
            "Zone.pop (mil)"
    ...
    source_pmsa_old = np.dot(
        source_concentrations_old, zone_populations_array
    ) / np.sum(zone_populations_array)
    source_pmsa_new = np.dot(
        source_concentrations_new, zone_populations_array
    ) / np.sum(zone_populations_array)

    pop_weighted_conc_new = np.dot(
        zone_concentrations_avg_new.T, zone_populations_array
    ) / np.sum(zone_populations_array)

    net_reduction = (
        100 * (pop_weighted_conc - pop_weighted_conc_new) / pop_weighted_conc
    )
    net_reduction_sourcewise = (
        100 * (source_pmsa_old - source_pmsa_new) / source_pmsa_old
    )
    ...
    source_pmsa_old_pct_fig = px.pie(
        values=list(100 * source_pmsa_old_pct.T.flatten()),
        names=sourceapportionment_df["Source"].to_list(),
        color=sourceapportionment_df["Source"].to_list(),
        color_discrete_map=sources_cmap,
    )

    source_pmsa_new_pct_fig = px.pie(
        values=list(100 * source_pmsa_new_pct.T.flatten()),
        names=sourceapportionment_df["Source"].to_list(),
        color=sourceapportionment_df["Source"].to_list(),
        color_discrete_map=sources_cmap,
    )
    ...
    costs_incurred = (
        costs_sourcewise_array * (source_pmsa_old.T - source_pmsa_new.T).flatten()
    )
    ...
    with c1:
        st.metric(
            label="Population weighted concentration - old",
        ...
        sourceapportionment_df = st.data_editor(
            sourceapportionment_df, column_config=column_config_dict
        )
        ...
        total_df = total_df[
            ["Source", "Z1", "Z2", "Z3", "Z4", "Z5", "Z6", "Z7", "Z8", "Z9", "Z10"]
        ]

        if all(
            [
                True if round(i) == 100 else False
                for i in sourceapportionment_df.sum(numeric_only=True).to_list()
            ]
        ):
            pass
        else:
            ...
            st.dataframe(
                total_df.style.format(precision=2).map(
                    style,
                    props="color:red;",
                    subset=[
                        "Z1",
            ...
            st.write(
                "Please check source apportionment values. Column Sum should be 100"
            )

        st.plotly_chart(source_pmsa_old_pct_fig, theme=None)

    with c2:
        st.metric(
            label="Population weighted concentration - new",
        ...
        sourceapportionment_new_df.columns = [
            "Z1",
            "Z2",
            "Z3",
            "Z4",
        ...
        sourceapportionment_new_df = sourceapportionment_new_df[
            ["Source", "Z1", "Z2", "Z3", "Z4", "Z5", "Z6", "Z7", "Z8", "Z9", "Z10"]
        ]
        ...
        st.plotly_chart(source_pmsa_new_pct_fig, theme=None)

        st.metric(label="Cost incurred", value=round(sum_cost_incurred))
...
with tab4:
    ...
    with container:
        ...
        max_reduction_industries = c2.slider(
            "Max reduction for Industries(%)", value=30
        )
        max_reduction_freight = c3.slider("Max reduction for Freight(%)", value=30)
        max_reduction_passtravel = c3.slider(
            "Max reduction for Pass Travel(%)", value=30
        )
    ...
    avg_pollution_old = sum(
        x * y for x, y in zip(zone_pollutions_old, zone_populations)
    ) / sum(zone_populations)
    ...
    pmsa_old = np.dot(zone_pollutions_old_sourcewise, zone_populations) / np.sum(
        zone_populations
    )

    max_reductions_sourcewise = {
        "C": max_reduction_cooking,
    ...
    pmsa_new_maxreduced = (
        pmsa_old * (100 - np.array(list(max_reductions_sourcewise.values()))) / 100
    )
    ...
    for i in range(1, num_zones + 1, 1):
        var_dict["R" + str(i) + "C"] = p.LpVariable(
            "R" + str(i) + "C", lowBound=0, upBound=max_reductions_sourcewise["C"] / 100
        )
        var_dict["R" + str(i) + "H"] = p.LpVariable(
            "R" + str(i) + "H", lowBound=0, upBound=max_reductions_sourcewise["H"] / 100
        )
        var_dict["R" + str(i) + "W"] = p.LpVariable(
            "R" + str(i) + "W", lowBound=0, upBound=max_reductions_sourcewise["W"] / 100
        )
        var_dict["R" + str(i) + "I"] = p.LpVariable(
            "R" + str(i) + "I", lowBound=0, upBound=max_reductions_sourcewise["I"] / 100
        )
        var_dict["R" + str(i) + "F"] = p.LpVariable(
            "R" + str(i) + "F", lowBound=0, upBound=max_reductions_sourcewise["F"] / 100
        )
        var_dict["R" + str(i) + "P"] = p.LpVariable(
            "R" + str(i) + "P", lowBound=0, upBound=max_reductions_sourcewise["P"] / 100
        )
        var_dict["R" + str(i) + "D"] = p.LpVariable(
            "R" + str(i) + "D", lowBound=0, upBound=max_reductions_sourcewise["D"] / 100
        )
        var_dict["R" + str(i) + "B"] = p.LpVariable(
            "R" + str(i) + "B", lowBound=0, upBound=max_reductions_sourcewise["B"] / 100
        )
    ...
    Lp_prob += (
        p.lpSum(
            [
                zone_populations[i - 1]
                * zone_pollutions_old_sourcewise[idx][i - 1]
                * var_dict["R" + str(i) + s]
    ...
    ) / sum(zone_populations) == (avg_pollution_old - avg_pollution_new)
    ...
    for idx, s in enumerate(["C", "H", "W", "I", "F", "P", "D", "B"]):
        Lp_prob += (
            p.lpSum(
                [
                    zone_populations[i - 1]
                    * zone_pollutions_old_sourcewise[0][i - 1]
                    * var_dict["R" + str(i) + s]
                    for i in range(1, num_zones + 1, 1)
                ]
            )
            / sum(zone_populations)
            <= pmsa_old[idx] - pmsa_new_maxreduced[idx]
        )
    ...
    reductions = [
        p.value(var_dict["R" + str(i) + s])
        for s in ["C", "H", "W", "I", "F", "P", "D", "B"]
        for i in range(1, num_zones + 1, 1)
    ]
    ...
    if status < 1:
        ...
        with c2:
            ...
            reduction_sourcewise_df.columns = [
                "Z1",
                "Z2",
                "Z3",
                "Z4",
            ...
            reduction_sourcewise_df = reduction_sourcewise_df.reset_index().rename(
        ...
        reduction_sourcewise_array = np.array(
            reduction_sourcewise_df.iloc[:, 1:].values
        )
        reduction_sourcewise_array = reduction_sourcewise_array / 100
        reduction_sourcewise_array = 1 - reduction_sourcewise_array
        zone_pollutions_new_sourcewise = (
            zone_pollutions_old_sourcewise * reduction_sourcewise_array
        )

        zone_concentrations_avg_new = np.sum(zone_pollutions_new_sourcewise, axis=0)

        zone_concentrations_df["Avg.conc ug/m3 (new)"] = (
            zone_concentrations_avg_new.round(1)
        )
        ...
        sourceapportionment_new = (
            zone_pollutions_new_sourcewise / zone_concentrations_avg_new
        )
        pmsa_old = np.dot(zone_pollutions_old_sourcewise, zone_populations) / np.sum(
            zone_populations
        )
        pmsa_new = np.dot(zone_pollutions_new_sourcewise, zone_populations) / np.sum(
            zone_populations
        )

        pop_weighted_conc_new = np.dot(
            zone_concentrations_avg_new.T, zone_populations
        ) / np.sum(zone_populations)

        c1, c2, c3, c4 = st.columns(4)
        with c1:
            st.metric(
                label="Pop. weighted concentration - old",
                value=round(avg_pollution_old, 1),
            )
        with c3:
            st.metric(
                label="Pop. weighted concentration - new",
        ...
        with c4:
            st.metric(
                ...
                value=round(
                    100
                    * (avg_pollution_old - pop_weighted_conc_new)
                    / avg_pollution_old,
                    1,
                ),
            )
        ...
        with c1:
            ...
            sourceapportionment_df = st.data_editor(
                sourceapportionment_df,
                column_config=column_config_dict,
                key="sourceapportionment_v4",
            )

            total_df = pd.DataFrame(
                sourceapportionment_df.sum(numeric_only=True)
            ).round()
            total_df = total_df.T
            total_df["Source"] = "Total              "
            total_df = total_df[
            ...
            ## PLOTLY FIGURE
            source_pmsa_old_pct_fig = px.pie(
                values=list(100 * pmsa_old.T.flatten()),
                names=sourceapportionment_df["Source"].to_list(),
                color=sourceapportionment_df["Source"].to_list(),
                color_discrete_map=sources_cmap,
            )
        ...
        with c2:
            ...
            zones = [zone for zone in zones if zone[0] == "Z"]
            for zone in zones:
                sourceapportionment_new_df[zone] = (
                    sourceapportionment_new_df[zone] * 100
                )
                column_config_dict[zone] = st.column_config.NumberColumn(
                    zone,
                    format="%.1f %%",
            ...
            st.data_editor(
                sourceapportionment_new_df,
                column_config=column_config_dict,
                key="sourceapportionment_new_v4",
            )

            ## PLOTLY FIGURE
            source_pmsa_new_pct_fig = px.pie(
                values=list(100 * pmsa_new.T.flatten()),
                names=sourceapportionment_df["Source"].to_list(),
                color=sourceapportionment_df["Source"].to_list(),
                color_discrete_map=sources_cmap,
            )
            ...
            costs_incurred = (
                costs_sourcewise_array * (pmsa_old.T - pmsa_new.T).flatten()
            )
            sum_cost_incurred = np.sum(costs_incurred)
            st.metric(label="Cost incurred", value=round(sum_cost_incurred))
