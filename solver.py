...
avg_conc_default = sum(x * y for x, y in zip(default_pollutions, populations))/sum(populations)
avg_conc_new = avg_conc_default*(1-net_reduction/100)


max_pollution_reduction = [40, 40, 40, 40, 30, 30, 30, 30, 30, 30] #This should be user input
...
max_pollution_reduced = calculate_pollution_after_reduction(default_pollutions, max_pollution_reduction)
...
for i in range(1,num_zones+1,1):
    var_dict['x'+str(i)] = p.LpVariable("x"+str(i), lowBound = max_pollution_reduced[i-1], upBound = default_pollutions[i-1])
...
print([p.value(var_dict['x'+str(i)]) for i in range(1,num_zones+1,1)], p.value(Lp_prob.objective))
